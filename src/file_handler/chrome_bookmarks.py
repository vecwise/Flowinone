...
def iter_chrome_bookmark_records():
    """Yield every Chrome bookmark with its display folder path."""
    bookmarks = _load_chrome_bookmarks()
    roots = bookmarks.get("roots", {})

    def _walk(node: dict, path_labels: List[str]):
...
def get_chrome_bookmarks(folder_path=None, focus_mode_id: Optional[str] = None):
    ...
    for child in children:
        ...
        if child_type == "folder":
            ...
            thumbnail_lookup = lookup_thumbnail_for_bookmark(url, child_name, folder_meta)
            thumbnail = thumbnail_lookup.route or DEFAULT_THUMBNAIL_ROUTE
            ...
            data.append(MediaEntry(
                ...
                item_path=url,
                media_type="bookmark",
                ext=thumbnail_lookup.sub_type,
                description=path_display or None,
                folder_labels=folder_labels,
                path_display=path_display,
                id=thumbnail_lookup.media_id,
                thumbnail_status=thumbnail_lookup.status,
                thumbnail_provider=thumbnail_lookup.provider,
            ))
...
def get_chrome_youtube_bookmarks():
    ...
    def _walk(node, path_labels):
        ...
        if node_type == "folder":
            ...
            thumbnail_lookup = lookup_thumbnail_for_bookmark(url, label, folder_meta)
            thumbnail = thumbnail_lookup.route or DEFAULT_THUMBNAIL_ROUTE
            sub_type = thumbnail_lookup.sub_type or "youtube"
